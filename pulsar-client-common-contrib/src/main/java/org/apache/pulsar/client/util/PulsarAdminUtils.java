...
public class PulsarAdminUtils {
    ...
    public static ConsumeStats getConsumeStats(String partitionTopic, int partition, String subscription,
        ...
        consumeStats.setLastConsumedOffset(
                extractMessageIndex(partitionTopic, parseMessageIdFromString(consumedPosition, partition),
                        brokerCluster, pulsarAdmin));
        consumeStats.setMaxOffset(
                extractMessageIndex(partitionTopic, parseMessageIdFromString(maxPosition, partition), brokerCluster,
                        pulsarAdmin));
        consumeStats.setMinOffset(
                extractMessageIndex(partitionTopic, parseMessageIdFromString(minPosition, partition), brokerCluster,
                        pulsarAdmin));
        return consumeStats;
    }
    ...
    private static long extractMessageIndex(String topic, MessageIdAdv messageId, String brokerCluster,
        ...
        if (ledgerId > 0 && entryId < 0) {
            entryId = 0;
            return getMessageIndex(topic, new MessageIdImpl(ledgerId, entryId, messageId.getPartitionIndex()), brokerCluster, pulsarAdmin) - 1;
        } else {
