                  ...
                  try {
                    ...
                    if (partitions <= 0) {
                      return createErrorResult(
                          "Invalid partitions parameter: " + "must be at least 1");
                    }
                    ...
                    return createErrorResult(
                        "Failed to update topic partitions: " + e.getMessage());
                  }
                })
            .build());
  }
                ...
                (exchange, request) -> {
                  try {
                    ...
                    return createSuccessResult(
                        "Compaction triggered successfully for topic: ", result);
                ...
                (exchange, request) -> {
                  try {
                    ...
                    Integer expireTimeInSeconds =
                        getIntParam(request.arguments(), "expireTimeInSeconds", 0);
                    ...
                    if (meta != null && meta.partitions > 0) {
                      for (int i = 0; i < meta.partitions; i++) {
                        pulsarAdmin
                            .topics()
                            .expireMessages(
                                topic + "-partition-" + i, subscriptionName, expireTimeInSeconds);
                      }
                    } else {
                      pulsarAdmin
                          .topics()
                          .expireMessages(topic, subscriptionName, expireTimeInSeconds);
                    }
                    ...
                    return createErrorResult(
                        "Failed to expire messages on topic: " + e.getMessage());
                  }
                })
            .build());
  }
            ...
            .callHandler(
                (exchange, request) -> {
                  try {
                    String topic = buildFullTopicName(request.arguments());
                    String subscription =
                        getRequiredStringParam(request.arguments(), "subscription");
            ...
            .callHandler(
                (exchange, request) -> {
                  try {
                    ...
                    result.put(
                        "lastLedgerCreatedTimestamp", internalStats.lastLedgerCreatedTimestamp);
                    result.put(
                    ...
                    if (internalStats.cursors != null && !internalStats.cursors.isEmpty()) {
                      ...
                      internalStats.cursors.forEach(
                          (name, cursor) -> {
                            ...
                            cursorInfo.put(
                                "messagesConsumedCounter", cursor.messagesConsumedCounter);
            ...
            .callHandler(
                (exchange, request) -> {
                  try {
                    ...
                    int partitions =
                        (partitionedMetadata == null) ? 0 : partitionedMetadata.partitions;
                    result.put("partitions", partitions);
                    result.put("isPartitioned", partitions > 0);
                    ...
                    if (partitions > 0) {
                      ...
                      for (int i = 0; i < partitions; i++) {
                        ...
                        try {
                          ...
                          int subCount =
                              (s.getSubscriptions() == null) ? 0 : s.getSubscriptions().size();
                    ...
                    addTopicBreakdown(result, topic);
                    return createSuccessResult(
                        "Partitioned metadata retrieved successfully", result);
                    ...
                    return createErrorResult(
                        "Failed to get partitioned metadata: " + e.getMessage());
                  }
                })
            .build());
  }
