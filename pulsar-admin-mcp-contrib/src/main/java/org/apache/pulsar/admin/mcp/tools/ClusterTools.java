            ...
            .callHandler(
                (exchange, request) -> {
                  try {
                    ...
                    List<String> dynamicNamesSorted =
                  ...
                  try {
                    ...
                    if (domainName != null && !domainName.isEmpty()) {
                      ...
                      for (Map.Entry<String, FailureDomain> e : raw.entrySet()) {
                        String dn = e.getKey();
                        Set<String> brokers =
                            (e.getValue() != null && e.getValue().getBrokers() != null)
                      ...
                      String msg =
                          "Cluster failure domains retrieved successfully"
                ...
                (exchange, request) -> {
                  try {
                    String clusterName =
                        getRequiredStringParam(request.arguments(), "clusterName").trim();
                    String domainName =
                        getRequiredStringParam(request.arguments(), "domainName").trim();
                    Integer minDomains = getIntParam(request.arguments(), "minDomains", 1);
                    boolean validate =
                        getBooleanParam(request.arguments(), "validateBrokers", true);

                    Object brokersObj = request.arguments().get("brokers");
                    if (!(brokersObj instanceof List<?> rawList)) {
                      return createErrorResult(
                          "Parameter 'brokers' must be a non-empty string list");
                    }
                    ...
                    for (int i = 0; i < rawList.size(); i++) {
                      ...
                      if (!(v instanceof String s) || (s = s.trim()).isEmpty()) {
                        return createErrorResult(
                            "All brokers must be non-empty strings "
                                + "(invalid at index "
                                + i
                                + ")");
                      }
                    ...
                    try {
                        ...
                        for (Map.Entry<String, FailureDomain> e : existing.entrySet()) {
                          ...
                          for (String b : brokerSet) {
                            if (used != null && used.contains(b)) {
                              return createErrorResult(
                                  "broker '" + b + "' already belongs to domain '" + dn + "'");
                            }
                          }
                        }
                      }
                      ...
                      domainObj = FailureDomain.builder().brokers(brokerSet).build();

                      if (isUpdate) {
                        pulsarAdmin
                            .clusters()
                            .updateFailureDomain(clusterName, domainName, domainObj);
                      } else {
                        pulsarAdmin
                            .clusters()
                            .createFailureDomain(clusterName, domainName, domainObj);
                      }
