...
public abstract class AbstractMCPServer {
  ...
  protected static void registerToolsConditionally(
      McpSyncServer mcpServer, Set<String> enabledTools, PulsarClientManager pulsarClientManager) {
    if (pulsarAdmin == null) {
      throw new RuntimeException("PulsarAdmin has not been initialized");
    }

    if (enabledTools.stream()
        .anyMatch(tool -> tool.contains("cluster") || tool.contains("broker"))) {
      registerToolGroup(
          "ClusterTools",
          () -> {
    ...
    if (enabledTools.stream()
        .anyMatch(
            tool ->
                tool.contains("namespace")
                    || tool.contains("retention")
                    || tool.contains("backlog"))) {
      registerToolGroup(
          "NamespaceTools",
          () -> {
    ...
    if (enabledTools.stream()
        .anyMatch(tool -> tool.contains("subscription") || tool.contains("unsubscribe"))) {
      registerToolGroup(
          "SubscriptionTools",
          () -> {
        ...
        .anyMatch(
            tool ->
                tool.contains("monitor")
                    ...
                    || tool.contains("backlog-analysis"))) {
      registerToolGroup(
          "MonitoringTools",
          () -> {
