                  ...
                  try {
                    ...
                    if (meta.partitions > 0) {
                      return createErrorResult(
                          "Please specify a concrete partition, e.g. topic-partition-0");
                    }
                    ...
                    result.put(
                        "consumerCount",
                        subStats.getConsumers() != null ? subStats.getConsumers().size() : 0);
            ...
            .callHandler(
                (exchange, request) -> {
                  try {
                    ...
                    MessageId initial =
                        pos.equals("earliest") ? MessageId.earliest : MessageId.latest;
            ...
            .callHandler(
                (exchange, request) -> {
                  try {
                    ...
                    return createErrorResult(
                        "Failed to reset subscription cursor: " + e.getMessage());
                  }
                })
            .build());
  }
                ...
                (exchange, request) -> {
                  try {
                    ...
                    Integer expireTimeSeconds =
                        getIntParam(request.arguments(), "expireTimeSeconds", 0);
                    ...
                    return createSuccessResult(
                        "Expired subscription messages up to message ID successfully", result);
                    ...
                    return createErrorResult(
                        "Failed to expire subscription messages: " + e.getMessage());
                  }
                })
            .build());
  }
            ...
            .callHandler(
                (exchange, request) -> {
                  try {
                    ...
                    if (meta.partitions > 0) {
                      ...
                      ps.getPartitions()
                          .forEach(
                              (partition, ts) -> {
                                var subStats =
                                    ts.getSubscriptions() != null
                                        ? ts.getSubscriptions().get(subscription)
                                        : null;
                      ...
                      var subStats =
                          stats.getSubscriptions() != null
                              ? stats.getSubscriptions().get(subscription)
                              : null;
                      if (subStats == null) {
                        return createErrorResult("Subscription not found: " + subscription);
                      }
                    ...
                    return createErrorResult(
                        "Failed to list subscription consumers: " + e.getMessage());
                  }
                })
            .build());
  }
                  ...
                  try {
                    ...
                    if (meta.partitions > 0) {
                      ...
                      for (String partition : ps.getPartitions().keySet()) {
                        try {
                          ...
                          if (internal != null
                            ...
                            positions.put(
                                partition, Map.of("message", "cursor not found on this partition"));
                          }
                        } catch (Exception ie) {
                      ...
                      if (internal != null
                        ...
                        return createErrorResult(
                            "Cursor not found for subscription: " + subscription);
                      }
                    }
                    ...
                    return createErrorResult(
                        "Failed to get subscription cursor positions: " + e.getMessage());
                  }
                })
            .build());
  }
