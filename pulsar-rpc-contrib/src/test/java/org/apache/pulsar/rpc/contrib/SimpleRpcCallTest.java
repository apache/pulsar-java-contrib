...
public class SimpleRpcCallTest extends PulsarRpcBase {
  private Function<PulsarRpcBase.TestRequest, CompletableFuture<PulsarRpcBase.TestReply>>
      requestFunction;
  ...
  public void testRpcCall() throws Exception {
    ...
    rpcServer =
        createPulsarRpcServer(
            pulsarClient, requestSubBase, requestFunction, rollbackFunction, null);
    ...
    for (int i = 0; i < messageNum; i++) {
      ...
      rpcClient
          .requestAsync(asyncCorrelationId, message)
          .whenComplete(
              (replyMessage, e) -> {
                if (e != null) {
                  log.error("error", e);
  ...
  public void testRpcCallWithCallBack() throws Exception {
    ...
    PulsarRpcServer<TestRequest, TestReply> rpcServer1 =
        createPulsarRpcServer(
            pulsarClient, requestSubBase + "-1", requestFunction, rollbackFunction, null);
    PulsarRpcServer<TestRequest, TestReply> rpcServer2 =
        createPulsarRpcServer(
            pulsarClient, requestSubBase + "-2", requestFunction, rollbackFunction, null);
    PulsarRpcServer<TestRequest, TestReply> rpcServer3 =
        createPulsarRpcServer(
            pulsarClient, requestSubBase + "-3", requestFunction, rollbackFunction, null);
  ...
  public void testRpcCallWithPattern() throws Exception {
    ...
    // 1.Create PulsarRpcClient
    rpcClient =
        createPulsarRpcClient(pulsarClient, requestProducerConfigMap, replyTopicPattern, null);
    ...
    rpcServer =
        createPulsarRpcServer(
            pulsarClient, requestSubBase, requestFunction, rollbackFunction, requestTopicPattern);
    ...
    for (int i = 0; i < messageNum; i++) {
      ...
      requestMessageConfigMap.put(TypedMessageBuilder.CONF_EVENT_TIME, System.currentTimeMillis());
      rpcClient
          .requestAsync(correlationId, message, requestMessageConfigMap)
          .whenComplete(
              (replyMessage, e) -> {
                if (e != null) {
                  log.error("error", e);
  ...
  public void testRpcCallTimeout() throws Exception {
    ...
    for (int i = 0; i < messageNum; i++) {
      ...
      try {
        TestRequest message = new TestRequest(synchronousMessage + i);
        requestMessageConfigMap.put(
            TypedMessageBuilder.CONF_EVENT_TIME, System.currentTimeMillis());
        TestReply reply = rpcClient.request(correlationId, message, requestMessageConfigMap);
        resultMap.put(correlationId, reply);
        log.info("[Synchronous] Reply message: {}, KEY: {}", reply.value(), correlationId);
  ...
  public void testDelayedRpcAt() throws Exception {
    ...
    for (int i = 0; i < messageNum; i++) {
      ...
      resultMap.put(correlationId, new AtomicInteger());
      rpcClient.requestAfterAsync(
          correlationId, message, requestMessageConfigMap, delayedTime, TimeUnit.MILLISECONDS);
    }
  ...
  private PulsarRpcClient<TestRequest, TestReply> createPulsarRpcClient(
      ...
      Map<String, Object> requestProducerConfigMap,
      Pattern replyTopicsPattern,
      RequestCallBack<TestReply> callBack)
    ...
    return replyTopicsPattern == null
        ? rpcClientBuilder.replyTopic(replyTopic).build(pulsarClient)
