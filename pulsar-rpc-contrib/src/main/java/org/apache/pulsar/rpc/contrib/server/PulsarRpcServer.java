 ...
 * request-response cycles in a distributed environment. This server is responsible for receiving
 * RPC requests, processing them, and sending the corresponding responses back to the client.
 *
 * <p>This class integrates tightly with Apache Pulsar's consumer and producer APIs to receive
 * messages and send replies. It uses a {@link GenericKeyedObjectPool} to manage a pool of Pulsar
 * producers optimized for sending replies efficiently across different topics.
 *
...
public interface PulsarRpcServer<T, V> extends AutoCloseable {
   ...
   */
  static <T, V> PulsarRpcServerBuilder<T, V> builder(
  ...
  /**
   * Closes the RPC server, releasing all resources such as the request consumer and reply producer
   * pool. This method ensures that all underlying Pulsar clients are properly closed to free up
   * network resources and prevent memory leaks.
   *
