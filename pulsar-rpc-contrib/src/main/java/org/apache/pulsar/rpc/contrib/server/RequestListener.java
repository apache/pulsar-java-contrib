...
/**
 * Handles incoming Pulsar messages, processes them using a specified function, and sends replies
 * using a {@link ReplySender}. This listener is typically used on the server side of a Pulsar RPC
 * implementation.
 *
...
class RequestListener<T, V> implements MessageListener<T> {
  ...
  /**
   * Processes received messages by applying a function to generate replies, which are then sent
   * back to the client. Handles request timeouts and errors during processing.
   *
    ...
    try {
      requestFunction
          ...
          .thenAccept(
              reply -> {
                sender.sendReply(
                    replyTopic, correlationId, reply, value, requestSubscription, delayedAt);
              })
          .get();
