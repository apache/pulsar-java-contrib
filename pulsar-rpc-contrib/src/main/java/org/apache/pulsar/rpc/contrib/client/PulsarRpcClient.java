...
/**
 * Provides the functionality to send asynchronous requests and handle replies using Apache Pulsar
 * as the messaging system. This client manages request-response interactions ensuring that messages
 * are sent to the correct topics and handling responses through callbacks.
 *
...
public interface PulsarRpcClient<T, V> extends AutoCloseable {
   ...
   */
  static <T, V> PulsarRpcClientBuilder<T, V> builder(
   ...
   * @throws PulsarRpcClientException if an error occurs during the request or while waiting for the
   *     reply.
   */
   ...
   * @param correlationId A unique identifier for the request.
   * @param value The value used to generate the request message
   * @param config Configuration map for creating a request producer, will call {@link
   *     TypedMessageBuilder#loadConf(Map)}
   * @return The reply value.
   * @throws PulsarRpcClientException if an error occurs during the request or while waiting for the
   *     reply.
   */
  V request(String correlationId, T value, Map<String, Object> config)
  ...
  /**
   * Deliver the message only at or after the specified absolute timestamp. Asynchronously sends a
   * request and returns a future that completes with the reply.
   *
   * @param correlationId A unique identifier for the request.
   * @param value The value used to generate the request message
   * @param timestamp Absolute timestamp indicating when the message should be delivered to
   *     rpc-server.
   * @return A CompletableFuture that will complete with the reply value.
   */
   ...
   * @return A CompletableFuture that will complete with the reply value.
   */
  CompletableFuture<V> requestAtAsync(
      String correlationId, T value, Map<String, Object> config, long timestamp);

  /**
   * Request to deliver the message only after the specified relative delay. Asynchronously sends a
   * request and returns a future that completes with the reply.
   *
   * @param correlationId A unique identifier for the request.
   * @param value The value used to generate the request message
   ...
   * @return A CompletableFuture that will complete with the reply value.
   */
  default CompletableFuture<V> requestAfterAsync(
      String correlationId, T value, long delay, TimeUnit unit) {
   ...
   * @return A CompletableFuture that will complete with the reply value.
   */
  CompletableFuture<V> requestAfterAsync(
   ...
   *
   * <p>When this method is executed, ReplyListener the received message will not be processed
   * again. You need to make sure that this request has been processed through the callback, or you
   * need to resend it.
   *
  ...
  /**
   * Closes this client and releases any resources associated with it. This includes closing any
   * active producers and consumers and clearing pending requests.
   *
