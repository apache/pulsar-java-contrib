...
class PulsarRpcClientImpl<T, V> implements PulsarRpcClient<T, V> {
    ...
    static <T, V> PulsarRpcClientImpl<T, V> create(
            ...
            @NonNull PulsarRpcClientBuilderImpl<T, V> builder) throws PulsarRpcClientException {
    ...
    private TypedMessageBuilder<T> newRequestMessage(String correlationId, T value, Map<String, Object> config) {
        TypedMessageBuilder<T> messageBuilder = requestProducer.newMessage().key(correlationId).value(value);
        if (!config.isEmpty()) {
