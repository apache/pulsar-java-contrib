...
/**
 * Builder class for creating instances of {@link PulsarRpcServer}. This class provides a fluent API
 * to configure the Pulsar RPC server with necessary schemas, topics, subscriptions, and other
 * configuration parameters related to Pulsar clients.
 *
 * <p>Instances of {@link PulsarRpcServer} are configured to handle RPC requests and replies using
 * Apache Pulsar as the messaging system. This builder allows you to specify the request and reply
 * topics, schemas for serialization and deserialization, and other relevant settings.
 *
...
public interface PulsarRpcServerBuilder<T, V> {
   ...
   * @return this builder instance
   */
  PulsarRpcServerBuilder<T, V> patternAutoDiscoveryInterval(
      @NonNull Duration patternAutoDiscoveryInterval);

  /**
   * Builds and returns a {@link PulsarRpcServer} instance configured with the current settings of
   * this builder. The server uses provided functional parameters to handle requests and manage
   * rollbacks.
   *
